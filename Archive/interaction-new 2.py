...
#convert files to dataframes using pandas
df = pd.read_excel(chatbot_file)
guided_df = pd.read_excel(guided_tour_file, sheet_name='Main')
faq_df = pd.read_excel(chatbot_file, sheet_name='FAQ')  
...
#show links via sidebar selection
...
if user_input:
    results = get_all_matches(user_input) 
    if 'messages' not in st.session_state:
...
for main_category in main_categories:
    with st.sidebar.expander(main_category, expanded=False):  # Create an expander for each COE
        ...
        default_option = "***Categories*** *(click to select):*"  
        ...
        # Only proceed if no user input is provided and a category is selected(to prevent double entries)
        if not user_input and selected_category != default_option:
            ...
            links_results = get_links_by_category(selected_category_name) 
